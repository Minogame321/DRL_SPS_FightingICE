...
class FightingiceEnv_Data_Frameskip(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        _actions = "AIR_A AIR_B AIR_D_DB_BA AIR_D_DB_BB AIR_D_DF_FA AIR_D_DF_FB AIR_DA AIR_DB AIR_F_D_DFA AIR_F_D_DFB AIR_FA AIR_FB AIR_GUARD AIR_UA AIR_UB BACK_JUMP BACK_STEP  CROUCH_A CROUCH_B CROUCH_FA CROUCH_FB CROUCH_GUARD FOR_JUMP FORWARD_WALK JUMP STAND_A STAND_B STAND_D_DB_BA STAND_D_DB_BB STAND_D_DF_FA STAND_D_DF_FB STAND_D_DF_FC STAND_F_D_DFA STAND_F_D_DFB STAND_FA STAND_FB STAND_GUARD THROW_A THROW_B"
    ...
    def _start_java_game(self):
        ...
        if self.system_name == "windows":
            ...
            self.java_env = subprocess.Popen(["java", "-Xms1024m", "-Xmx1024m", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--py4j", "--fastmode",
                                          "--grey-bg", "--inverted-player", "1", "--limithp", "400", "400"], stdout=devnull, stderr=devnull)
            ...
            self.java_env = subprocess.Popen(["java", "-cp", self.start_up_str, "Main", "--port", str(self.port), "--py4j","--fastmode",
                                             ...
                                             "--grey-bg","--inverted-player", "1", "--mute","--limithp", "10000", "10000"], stdout=devnull, stderr=devnull)
        elif self.system_name == "macos":
            self.java_env = subprocess.Popen(["java", "-XstartOnFirstThread", "-cp", "Main", "--port", str(self.port), "--py4j", "--fastmode",
    ...
    def _start_gateway(self, p2=Machete):
        ...
        if self.port==5003:
           self.p2 = p2
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "DiceAI", self.freq_restart_java)
        elif self.port==1112:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "Toothless", self.freq_restart_java)
        elif self.port==5005:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "BCP", self.freq_restart_java)
        elif self.port==5006:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "BCP", self.freq_restart_java)
        ...
        #   self.p2 = p2(self.gateway)
        #   self.manager.registerAI(self.p2.__class__.__name__, self.p2)
        #   self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "FICE_AI_OM", self.freq_restart_java)
        """
        elif self.port==5006:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "Dora", self.freq_restart_java)
        elif self.port==5007:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "MogakuMono", self.freq_restart_java)
        elif self.port==5008:
           self.p2 = p2(self.gateway)
           self.manager.registerAI(self.p2.__class__.__name__, self.p2)
           self.game_to_start = self.manager.createGame("ZEN", "ZEN", self.p1.__class__.__name__, "HaibuAI", self.freq_restart_java)
        """
       #---learning---
    ...
    def reset(self, p2=Machete):
        ...
        self.round_num += 1
        obs = self.pipe.recv()
        #print("obs in env_reset="+str(obs))
        return obs,0
